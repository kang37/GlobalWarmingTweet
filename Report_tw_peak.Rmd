...
将这些起点日期表示在之前的90%高值日期图中：

```{r fig.height=15}
subset(tw.ana, int_idx == "int90") %>% 
  ggplot() + 
  geom_tile(aes(as.factor(month), day, fill = comp), alpha = 0.3) + 
  theme(axis.text.x = element_text(angle = 90)) + 
  geom_tile(data = tw.highcontinue, aes(as.factor(month), day, fill = comp)) +
  facet_grid(year ~ dt_set)
```

上图对应的数据为，其中“TRUE”为该口径下该日期为高值起点日期：

```{r}
tw.highcontinue %>% 
  select(date, dt_set) %>% mutate(val = TRUE) %>% 
  pivot_wider(id_cols = date, names_from = dt_set, values_from = val) %>% 
  mutate(tw_gw_cc = (tw + gw + cc) == 3, 
         tw_gw = (tw + gw) == 2, 
         tw_cc = (tw + cc) == 2, 
         gw_gw = (gw + gw) == 2) %>% 
  knitr::kable()
...
通过可视化，可以看出CC和GW及TW的表现有所不同，3者同时为高值的情况很罕见。此外，图中的数字表示该起点日期对应的高值天数。
...
tw.highcontinue %>% 
  ...
  subset(high_days >= 3) %>% 
  ggplot() + 
  geom_tile(aes(dt_set, date, fill = comp)) + 
  facet_wrap(.~ year, scales = "free")
```

另外，关于要选择连续几天高值作为分析对象：目前选择的是3天及以上就算峰值。下图展示3个口径下，超过n天连续高值的峰值数所占比例。从数字来看，如果照目前标准（3天以上），GW和TW均有40左右个峰值；如果把范围限制在4天及以上，那CC就有22个左右峰值，GW和TW就是28左右个峰值。
